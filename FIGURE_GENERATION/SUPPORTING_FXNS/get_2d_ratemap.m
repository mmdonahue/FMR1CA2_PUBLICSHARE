...
% EDIT 08/2022: Changed an error that caused spike times to not be velocity
% filtered correctly.
                ...
                if velFilt == 1 %if only looking at firing rate while the rat is moving
                    if ~isempty(spkInds)
                        for si = 1:length(spkInds)
                            spkTmInd = find(coords(:,1)>=spkTms(spkInds(si)), 1, 'First'); % doing '>=' because of the half-frame time adjustment above
